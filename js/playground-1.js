...
function highlightslice(text) {
	var c = currentstring.value;
	var inputText = document.getElementById("testinfo");
	var innerHTML = inputText.innerHTML;
	var index = innerHTML.indexOf(text);
	if (index >= 0) {
		innerHTML = innerHTML.substring(0, index) +
			"<span class='highlight'>" + innerHTML.substring(index, index + text.length) +
			"</span>" + innerHTML.substring(index + text.length);
		inputText.innerHTML = innerHTML;
		
		//$(innerHTML).css('color', 'red');
	}
	//else {

	//	innerHTML = innerHTML.substring(0, index) +
	//		"<span class='lowlight'>" + innerHTML.substring(index, index + text.length) +
	//		"</span>" + innerHTML.substring(index + text.length);
	//	inputText.innerHTML = innerHTML;
...
function playseq() {
	...
	if (n === "") {
		stoptimeout();
		document.getElementById("indexvalue").value = "end8";
		loopseq();

		// Uncomment for continuous play
		//nextheight();
	}
...
function clickthruseq() {	
	...
	if (n === "") {
		document.getElementById("indexvalue").value = "end";
		resetslice();
	}
}
