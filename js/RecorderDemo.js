(function () {
	...
	testTone = (function () {
		...
		lfo = Tone.context.createOscillator();
		lfo.type = 'square';
		osc.frequency.value = "66";
		oscMod = Tone.context.createGain();
		osc.connect(oscMod);
		lfo.connect(oscMod.gain);
		output = Tone.context.createGain();
		output.gain.value = 0.5;
		oscMod.connect(output);
		osc.start();
		lfo.start();
		//document.getElementById("testTB").value = instrument;

		return output;
	})();

	testToneLevel = Tone.context.createGain();
